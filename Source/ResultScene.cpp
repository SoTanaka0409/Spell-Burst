...
/// @brief DrawOutlinedString を実行する
/// @param x x の値
/// @param y y の値
/// @param str str の値
/// @param color color の値
/// @param outlineColor outlineColor の値
/// @param fontHandle fontHandle の値
...
void ResultScene::Draw()
{
	...
	if (state_timer_ > 90)
	{
		...
		DrawOutlinedString(350, 600, "決定キー・クリックでタイトルへ戻る", GetColor(255, 255, 255), GetColor(0, 0, 0), promptFont);
		SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);
	}
}

/// @brief DrawVictory を実行する
void ResultScene::DrawVictory()
{
	...
	DrawOutlinedString(500, titleY, "ゲームクリア！！", GetColor(255, 255, 0), GetColor(255, 100, 0), titleFont);
	if (state_timer_ > 60)
	{
		DrawOutlinedString(550, titleY + 120, "プレイありがとうございました", GetColor(255, 255, 255), GetColor(0, 0, 100), subFont);
	}
}

/// @brief DrawGameOver を実行する
void ResultScene::DrawGameOver()
{
	...
	DrawOutlinedString(350, titleY, "ゲームオーバー", GetColor(255, 50, 50), GetColor(50, 0, 0), titleFont);
	if (state_timer_ > 60)
	{
		DrawOutlinedString(400, titleY + 120, "もう一度挑戦しよう", GetColor(200, 200, 200), GetColor(50, 50, 50), subFont);
	}
}
