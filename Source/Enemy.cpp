...
/// @brief UpdateAttackPattern を実行する
...
/// @brief DrawHpBar を実行する
void Enemy::DrawHpBar()
{
	int hpOffset = (enemy_type_ == 4) ? 65 : 55;
	DrawFormatString(static_cast<int>(position_.x) - 15, static_cast<int>(position_.y) - hpOffset, GetColor(255, 255, 255), "体力:%d", hp_);
}
