...
private:
    std::unique_ptr<EnemyManager> enemyManager;   // 敵の生成や進行フェーズを管理するマネージャーのポインタ

    int cutinTimer;               // カットイン演出（大技発動時など）の進行度を測るタイマー
    int cutinImageHandle;         // カットイン用画像のグラフィックハンドル（DxLib用）

    int screenHandle;             // オフスクリーン描画（画面揺れ等のポストエフェクト用）の画像ハンドル

    int shakeTimer;               // 画面揺れ（スクリーンシェイク）の残りフレーム数
    float shakeMagnitude;         // 画面揺れの強さ（ピクセル幅など）

    int hitStopTimer;             // ヒットストップ（一時的な時間停止演出）の残りフレーム数
...
public:
    ...
    // [入力] なし
    // [出力] なし
    // [副作用] オフスクリーン(screenHandle)に一度描画した後、画面揺れ(シェイク)のズレを加味してメイン画面へ転送する
    void Draw() override;
